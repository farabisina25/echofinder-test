...
Do NOT use markdown code blocks for the JSON output. Just raw JSON.`;
                    ...
                    console.log('🤖 Generating merge preview with Gemini...');
                    const result = await model.generateContent(prompt);
                    const text = result.response.text().replace(/^```json/, '').replace(/```$/, '').trim();
                    const mergedData = JSON.parse(text);

                    previewContent = `\n\n<details>\n<summary>✨ <strong>Click to see Proposed Merged Issue</strong></summary>\n\n` +
                        `### ${mergedData.title}\n\n${mergedData.body}\n\n` +
            ...
            const originalIssueNotice = `🔔 Note: A new issue (${repoName}#${issue.number}) was opened that appears to be a possible duplicate of this issue.\n\n` +
